 ...
 *
 * @param {object} json, contains a json file with the meeting attributes.
...
module.exports.create = function (json, callback) {
    ...
    requestAuthentication.create(endPoint, json, function (err, res) {
 ...
 * @param {object}jsonUpdate, contains a json file with the meeting attributes that will be updated.
 * @param{function} callback, contains the error and the response.
 */
module.exports.update = function (idMeeting, jsonUpdate, callback) {
    var endPoint = resource.services + '/' + service.id + resource.rooms + '/' + room.id + resource.meeting + '/' + idMeeting;
    requestAuthentication.update(endPoint, jsonUpdate, function (err, res) {
